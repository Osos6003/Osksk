...
def ytdownloaderHandler(c,m):
    ...
    channel = r.get(f'{Dev_Naif}:BotChannel') if r.get(f'{Dev_Naif}:BotChannel') else 'w7G_BoT'
    Thread(target=yt_func,args=(c,m,k,channel)).start()
    
def yt_func(c,m,k,channel):
   if not r.get(f'{m.chat.id}:enable:{Dev_Naif}'):
        return False 
   if r.get(f'{m.from_user.id}:mute:{m.chat.id}{Dev_Naif}'):  return False
   if r.get(f'{m.chat.id}:mute:{Dev_Naif}') and not admin_pls(m.from_user.id,m.chat.id):  return False 
   if r.get(f'{m.from_user.id}:mute:{Dev_Naif}'):  return False 
   text = m.text
   ...
   if text.startswith('يوت '):
     if r.get(f'{m.chat.id}:disableYT:{Dev_Naif}'):  return
     if r.get(f':disableYT:{Dev_Naif}'):  return
   ...
   if text.startswith('ساوند '):
     if r.get(f'{m.chat.id}:disableSound:{Dev_Naif}'):  return
     if r.get(f':disableYT:{Dev_Naif}'):  return
   ...
   if text.startswith('تيك '):
     if r.get(f'{m.chat.id}:disableTik:{Dev_Naif}'):  return
     if r.get(f':disableYT:{Dev_Naif}'):  return
     if Find(text):
       query = Find(text)[0]
     else:  return False
...
async def shazamFunc(c,m):
   if r.get(f'{m.chat.id}:disableShazam:{Dev_Naif}'):  return False
...
def getInfo(c, query):
    ...
    if r.get(f'{query.message.chat.id}:disableYT:{Dev_Naif}'):  return
    if r.get(f':disableYT:{Dev_Naif}'):  return
