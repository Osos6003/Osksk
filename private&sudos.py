...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      if m.from_user.id != to and m.from_user.id != data["from"] and m.from_user.id != 5117901887 and m.from_user.id != 6168217372:
         return await m.reply("☆ الهمسه لا تخصك ححححبيبي")
      else:
         if file:
...
async def to_send(c: Client, m: Message):
   ...
   if r.get(f'{m.chat.id}:pvBroadcast:{m.from_user.id}{Dev_Naif}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:pvBroadcast:{m.from_user.id}{Dev_Naif}')
      if m.text and m.text == 'الغاء':
         return await m.reply(f"{k} ابشر الغيت كل شي")
      users = r.smembers(f'{Dev_Naif}:UsersList')
      count = 0
      failed = 0
   ...
   k = r.get(f'{Dev_Naif}:botkey')
   if r.get(f'{m.chat.id}:gpBroadcast:{m.from_user.id}{Dev_Naif}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:gpBroadcast:{m.from_user.id}{Dev_Naif}')
      if m.text and m.text == 'الغاء':
         return await m.reply(f"{k} ابشر الغيت كل شي")
      chats = r.smembers(f'enablelist:{Dev_Naif}')
      count = 0
      failed = 0
      rep = await  m.reply("جار الاذاعة..")
      for chat in chats:
...
''', reply_markup=InlineKeyboardMarkup ([
  ...
  [InlineKeyboardButton (f'تحديثات {name} 🍻', url=f'https://t.me/{channel}')]
     ...
     if not r.sismember(f'{Dev_Naif}:UsersList',m.from_user.id):
       r.sadd(f'{Dev_Naif}:UsersList',m.from_user.id)
...
☆ عدد المستخدمين صار {}
'''.format(m.from_user.mention,m.from_user.id,username,len(r.smembers(f'{Dev_Naif}:UsersList')))
       reply_markup = InlineKeyboardMarkup ([[InlineKeyboardButton (m.from_user.first_name, user_id=m.from_user.id)]])
       if r.get(f'DevGroup:{Dev_Naif}'):
          c.send_message(
          int(r.get(f'DevGroup:{Dev_Naif}')),
...
def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if (r.get(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Naif}') or r.get(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Naif}') or r.get(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Naif}') or r.get(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Naif}') or r.get(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Naif}')) and text == 'الغاء':
       m.reply(quote=True,text=f'{k} من عيوني لغيت كل شي')
       r.delete(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Naif}')
       r.delete(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Naif}')
       r.delete(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Naif}')
       r.delete(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Naif}')
       return r.delete(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Naif}')

   if r.get(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Naif}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setBotName:{m.from_user.id}{Dev_Naif}')
      r.set(f'{Dev_Naif}:BotName',m.text)
      return m.reply(quote=True,text=f'{k} ابشر عيني المطور غيرت اسمي لـ {m.text}')
   
   if r.get(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Naif}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setBotChannel:{m.from_user.id}{Dev_Naif}')
      r.set(f'{Dev_Naif}:BotChannel',m.text.replace('@',''))
      return m.reply(quote=True,text=f'{k} ابشر عيني غيرت قناة السورس لـ {m.text}')
   
   if r.get(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Naif}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setBotKey:{m.from_user.id}{Dev_Naif}')
      r.set(f'{Dev_Naif}:botkey',m.text)
      return m.reply(quote=True,text=f'{k} ابشر عيني غيرت رمز السورس لـ {m.text}')
      
   if r.get(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Naif}') and devp_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setDevGroup:{m.from_user.id}{Dev_Naif}')
      try:
        id = int(m.text)
      except:
        return m.reply(quote=True,text=f'{k} الايدي غلط!')
      r.set(f'DevGroup:{Dev_Naif}', int(m.text))
      return m.reply(quote=True,text=f'{k} ابشر عيني قروب المطور لـ {m.text}')
   
   if r.get(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Naif}') and devp_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Naif}')
      try:
        get = c.get_chat(m.text.replace('@',''))
      except:
        return m.reply(quote=True,text=f'{k} اليوزر غلط!')
      r.set(f'{Dev_Naif}botowner', get.id)
   ...
   if text == 'المستخدمين المحظورين' or text == 'المحظورين عام':
     if not dev_pls(m.from_user.id, m.chat.id):
        ...
        if not r.smembers(f'listGBAN:{Dev_Naif}'):
           ...
           count = 1
           for user in r.smembers(f'listGBAN:{Dev_Naif}'):
               try:
   ...
   if text == 'المجموعات المحظورة':
     if not dev2_pls(m.from_user.id, m.chat.id):
        ...
        if not r.smembers(f':BannedChats:{Dev_Naif}'):
           return m.reply(quote=True,text=f'{k} مافي قروب محظور عام')
        else:
           text = 'المجموعات المحظورة عام:\n'
           count = 1
           for user in r.smembers(f':BannedChats:{Dev_Naif}'):
               text += f'{count}) {user}\n'
               count += 1
           return m.reply(quote=True,text=text)
   ...
   if text == 'مجموعة المطور' and m.chat.type == ChatType.PRIVATE:
     if not dev_pls(m.from_user.id,m.chat.id):
        ...
        if not r.get(f'DevGroup:{Dev_Naif}'):
           return m.reply(quote=True,text=f'{k} مجموعة المطور مو معينة')
        else:
           id = int(r.get(f'DevGroup:{Dev_Naif}'))
   ...
   if text == 'تغيير المطور الاساسي':
     if not devp_pls(m.from_user.id,m.chat.id):
        ...
        r.set(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Naif}',1,ex=600)
   ...
   if text == 'اذاعة بالخاص':
      if not dev2_pls(m.from_user.id,m.chat.id):
         return 
      r.set(f'{m.chat.id}:pvBroadcast:{m.from_user.id}{Dev_Naif}',1,ex=300)
      return m.reply(f"{k} ارسل الاذاعة الحين")

   if text == 'اذاعة بالقروبات':
      if not dev2_pls(m.from_user.id,m.chat.id):
         return 
      r.set(f'{m.chat.id}:gpBroadcast:{m.from_user.id}{Dev_Naif}',1,ex=300)
      return m.reply(f"{k} ارسل الاذاعة الحين")
   ...
   if text == 'جلب نسخة القروبات' and devp_pls(m.from_user.id,m.chat.id):
     list = []
     date = datetime.now()
     for chat in r.smembers(f'enablelist:{Dev_Naif}'):
   ...
   if text == 'جلب نسخة المستخدمين' and devp_pls(m.from_user.id,m.chat.id):
     list = []
     date = datetime.now()
     for chat in r.smembers(f'{Dev_Naif}:UsersList'):
        list.append(int(chat))
     with open(f'{date}.json', 'w+') as w:
   ...
   if text == 'المكتومين عام':
      if not dev_pls(m.from_user.id,m.chat.id):
        ...
        if not r.smembers(f'listMUTE:{Dev_Naif}'):
          ...
          text = '- المكتومين عام:\n\n'
          count = 1
          for PRE in r.smembers(f'listMUTE:{Dev_Naif}'):
             if count == 101: break
             try:
